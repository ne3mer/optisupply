/**
 * Generate 4 CSV files, each with 40 suppliers (160 total)
 * Output: 4 CSV files ready for bulk import
 */
...
function generateCompanyName(index, fileIndex) {
  ...
  // Add file index to ensure uniqueness across files
  return `${prefix} ${industry} ${suffix} ${type} (Batch ${fileIndex + 1})`.trim();
}
...
function generateSupplier(index, fileIndex) {
  ...
  // Use global index (fileIndex * 40 + index) to ensure different distribution per file
  const globalIndex = fileIndex * 40 + index;
  const riskTier = globalIndex % 10;
  let baseRiskValue;
  if (riskTier < 3) {
  ...
  const supplier = {
    name: generateCompanyName(index, fileIndex),
...
// Generate 4 files, each with 40 suppliers
const outputDir = path.join(__dirname, "../data");
if (!fs.existsSync(outputDir)) {
  fs.mkdirSync(outputDir, { recursive: true });
}

console.log("🚀 Generating 4 CSV files with 40 suppliers each...\n");

for (let fileIndex = 0; fileIndex < 4; fileIndex++) {
  const suppliers = [];
  for (let i = 0; i < 40; i++) {
    suppliers.push(generateSupplier(i, fileIndex));
  }

  const csvContent = generateCSV(suppliers);
  const filename = `suppliers_batch_${fileIndex + 1}_40.csv`;
  const outputPath = path.join(outputDir, filename);
  
  fs.writeFileSync(outputPath, csvContent, "utf8");

  console.log(`✅ Batch ${fileIndex + 1}: Generated ${suppliers.length} suppliers`);
  console.log(`   📁 Saved to: ${outputPath}`);
  console.log(`   📊 Sample: ${suppliers[0].name} (${suppliers[0].country}, ${suppliers[0].industry})`);
  console.log(`   📈 Score range: ${Math.min(...suppliers.map(s => s.finalScore)).toFixed(1)} - ${Math.max(...suppliers.map(s => s.finalScore)).toFixed(1)}\n`);
}

console.log("✨ All 4 files generated successfully!");
console.log("📦 Total: 160 suppliers across 4 files");
console.log("\n📋 Files ready for bulk import:");
for (let i = 1; i <= 4; i++) {
  console.log(`   - suppliers_batch_${i}_40.csv`);
}
